...
class Votmcav {
    ...
    public function getAll($idcen = null) {
        $sql = "SELECT u.idusu, u.ndocusu, u.nomusu, u.idper, u.noca, p.nomper, uf.idfic, f.nomfic, v.nomval, u.emausu, u.fotcan, u.telcan, c.nomcen, u.actusu 
                FROM usuario AS u 
                INNER JOIN usupef AS up ON u.idusu = up.idusu 
                INNER JOIN perfil AS p ON p.idper = up.idper 
                INNER JOIN centro AS c ON u.idcen = c.idcen 
    ...
    public function save() {
        try {
            ...
            error_log("Error al registrar candidato vocero: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function update() {
        try {
            ...
            error_log("Error al actualizar candidato vocero: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function delete() {
        try {
            ...
            // 1. Eliminar asignación de rol candidato vocero en usupef
            $sql1 = "DELETE FROM usupef WHERE idusu = :idusu AND idper = 13";
            $stmt1 = $conexion->prepare($sql1);
            $stmt1->bindParam(":idusu", $this->idusu, PDO::PARAM_INT);
            $stmt1->execute();

            // 2. Liberar número de tarjetón, foto y teléfono sin desactivar al aprendiz
    ...
    public function getVotosByCandidato($idcandidato) {
        $sql = "SELECT COUNT(*) as total FROM voto WHERE canusu = :canusu AND tipo_voto = 'vocero'";
        $modelo = new conexion();
        $conexion = $modelo->get_conexion();
        $result = $conexion->prepare($sql);
    ...
    public function checkNocaExists($noca, $excludeId = null) {
        $sql = "SELECT COUNT(*) as total FROM usuario u INNER JOIN usupef up ON u.idusu = up.idusu WHERE u.noca = :noca AND up.idper = 13";
        if ($excludeId) {
    ...
    public function buscarPorDocumento($ndocusu) {
        $sql = "SELECT u.idusu, u.ndocusu, u.nomusu, u.emausu, u.telcan, u.fotcan, u.noca,
                       ...
                       (SELECT COUNT(*) FROM usupef up WHERE up.idusu = u.idusu AND up.idper = 13) AS es_candidato
                FROM usuario AS u 
    ...
    public function asignarFicha($idusu, $idfic) {
        try {
            ...
            error_log("Error al asignar ficha al candidato vocero: " . $e->getMessage());
            return false;
        }
    }

    public function getResultadosVotacion($idfic) {
        $sql = "SELECT u.idusu, u.nomusu, u.noca, u.fotcan, f.nomfic, COUNT(v.id) as total_votos
                FROM usuario u
                INNER JOIN usupef up ON u.idusu = up.idusu
                INNER JOIN usufic uf ON u.idusu = uf.idusu
                INNER JOIN ficha f ON uf.idfic = f.idfic
                LEFT JOIN voto v ON v.canusu = u.idusu AND v.tipo_voto = 'vocero'
                WHERE uf.idfic = :idfic
                AND up.idper = 13
                AND u.actusu = 1
                GROUP BY u.idusu, u.nomusu, u.noca, u.fotcan, f.nomfic
